...
- **CLOSED — superseded by architecture**: whether `anon`/`authenticated` retain any direct `UPDATE` privilege on `trip_invites` was re-checked live against production and confirmed structurally impossible, not merely policy-restricted. Evidence: (1) `information_schema.role_table_grants` shows no `UPDATE` grant for `anon` or `authenticated` on `trip_invites` — only `postgres`/`service_role` have it; (2) `pg_policy` shows only `INSERT`/`DELETE`/`SELECT` policies exist on the table, no `UPDATE` policy at all, so even a grant would have nothing to permit it under RLS; (3) the legitimate status-transition paths (`accept_trip_invite`, `decline_trip_invite`, `revoke_trip_invite`) are `SECURITY DEFINER` functions rather than direct client writes. No further action needed on this concern.
